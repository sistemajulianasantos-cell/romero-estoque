...
function extrairDadosContratoRomero(txt) {
  ...
  // Tabela: BEBIDA | ESTIMATIVA — bebidas que o cliente fornece
  {
    const mL = t.match(/l\)[^\n]*destilados?[^\n]*\n([\s\S]*?)(?=\nm\)|\nCLÁUSULA|\n[a-z]\))/i);
    if (mL) {
      const linhas = mL[1].split('\n')
        .map(l => l.trim())
        .filter(l => l && !/^BEBIDA$|^ESTIMATIVA$|^VALOR$/i.test(l))
        .filter(l => !/^R\$|^\d+\s+A\s+\d+/i.test(l));
      if (linhas.length) d.bebidasCliente = linhas.join('\n').trim();
    }
  }

  // ── BEBIDAS ROMERO ou CONSIGNADAS — cláusula 2, letra m) ────────────────
  // Se contém "CONSIGNADA" → bebidasRomero (campo Consignadas)
  // Se não contém → bebidasRomero (campo Bebidas Romero)
  {
    const mM = t.match(/m\)[^\n]*Bebidas fornecidas pela ROMERO[^\n]*\n([\s\S]*?)(?=\nn\)|\no\)|\nCLÁUSULA|\nAssinatura|$)/i);
    if (mM) {
      const cabecalho = mM[0].split('\n')[0] || '';
      const isConsignada = /CONSIGNADA/i.test(cabecalho);
      const linhas = mM[1].split('\n')
        .map(l => l.trim())
        .filter(l => l && !/^BEBIDA$|^ESTIMATIVA$|^VALOR$/i.test(l))
        .filter(l => !/^R\$\s*\d/i.test(l))
        .map(l => l.replace(/\s+R\$[\d.,]+\s*$/,'').replace(/\s+\d+\s+A\s+\d+\s+GARRAFAS?\s*$/i,'').trim())
        .filter(l => l.length > 2);
      if (linhas.length) {
        if (isConsignada) {
          d.bebidasConsignadas = linhas.join('\n').trim();
        } else {
          d.bebidasRomero = linhas.join('\n').trim();
        }
      }
    } else {
