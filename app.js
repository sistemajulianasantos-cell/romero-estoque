...
function extrairDadosContratoRomero(txt) {
  ...
  // "sendo o início às HH:HH/HHhHH do dia ... e término às HHhHH"
  const mH = t.match(/sendo o início às\s+(\d{1,2})[Hh:](\d{2})\w*\s+do dia[\s\S]{1,40}?e\s+término às\s+(\d{1,2})[Hh:](\d{2})/i);
  if (mH) {
  ...
  if (mBloco) {
    ...
    while ((em = eqRe.exec(mBloco[1])) !== null) {
      d.equipe.push({ qtd: parseInt(em[1]), cargo: em[2].replace(/s\s*$/i,'').trim() });
    }
  }
  ...
  // ── CARDÁPIO ───────────────────────────────────────────────────────────────
  const mCard = txt.match(/(?:Coquet[eé]is?:|Recpetivo|Receptivo)([\s\S]*?)(?=\nf\)|\ng\)|\nCLÁUSULA)/i);
  if (mCard) d.cardapio = mCard[0].replace(/\s{3,}/g,'\n').trim();
...
async function preencherFormImportacao(txt, nomeArquivo) {
  document.getElementById('imp-status').textContent = 'Extraindo dados com IA…';
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `Extraia os dados deste contrato de serviços de bar e retorne APENAS um JSON válido, sem markdown, sem explicações.

Campos a extrair:
- contratante: nome completo do contratante (primeira pessoa/empresa mencionada, antes de "portadora" ou "portador" ou "doravante")
- cpf: CPF ou CNPJ do contratante
- tipo: tipo do evento (Casamento, Noivado, Aniversário, Formatura, Festa Junina, Corporativo, etc)
- nomeEvento: nome descritivo do evento (ex: "Casamento entre João e Maria", "Noivado da Victória e Patrick")
- data: data do evento no formato YYYY-MM-DD
- convidados: número de convidados (só o número)
- local: endereço completo do evento (nome do espaço + endereço + cidade)
- hrInicio: horário de início no formato HH:MM (da Cláusula 2b "sendo o início às...")
- hrFim: horário de término no formato HH:MM (da Cláusula 2b "término às...")
- duracao: duração em horas (ex: "8h")
- equipe: array de objetos [{qtd: número, cargo: "nome do cargo"}] em ordem: Head Bartender, Bartender, Bar Back, Copeiro
- valorTotal: valor total sem R$ e sem texto (ex: "8.700,00")
- parcelas: array [{valor: "1.740,00", descricao: "Sinal — assinatura do contrato", vencimento: "YYYY-MM-DD"}, {valor: "6.960,00", descricao: "Restante — venc. DD/MM/AAAA", vencimento: "YYYY-MM-DD"}]
- horaExtra: valor da hora extra sem R$ (ex: "1.340,00")
- cardapio: lista de coquetéis e serviços
- bebidasRomero: bebidas fornecidas pela Romero
...
${txt.slice(0, 6000)}`
    ...
    set('imp-fim',    d.hrFim);
    set('imp-equipe', d.equipe ? d.equipe.map(e=>`${e.qtd} ${e.cargo}`).join(' · ') : '');
    set('imp-valor',  d.valorTotal);

    // Calcular equipeTexto e total
    d.equipeTexto = d.equipe ? d.equipe.map(e=>`${e.qtd} ${e.cargo}`).join(' · ') : '';
    d.equipeTotal = d.equipe ? d.equipe.reduce((s,e)=>s+(e.qtd||0),0) : 0;
    ...
    if (resumo) {
      resumo.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:6px;font-size:12px">
        ...
        ${d.parcelas?.length?`<span class="tag tag-green">💰 ${d.parcelas.length} parcela(s)</span>`:''}
      </div>`;
      resumo.style.display='';
    }

    document.getElementById('imp-status').textContent = 'Dados extraídos com IA ✓ — confirme e ajuste se necessário.';
    document.getElementById('imp-form').style.display = '';

  } catch(e) {
    console.error('Erro na extração:', e);
    // Fallback: extração manual
    document.getElementById('imp-status').textContent = 'Extração automática falhou — preenchendo o que foi possível.';
