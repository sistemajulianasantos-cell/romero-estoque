...
const ACESSO = {
  ...
  financeiro:  ['contratos','producao','separacao','agenda','financeiro'],
...
function imprimirSeparacao(id) {
  ...
  <style>body{font-family:Arial,sans-serif;font-size:11px;margin:20px;color:#111}h2{font-size:14px;margin:0 0 2px}.sub{font-size:11px;color:#555;margin-bottom:12px}table{width:100%;border-collapse:collapse;margin-bottom:14px}th{background:#111;color:#fff;padding:5px 8px;text-align:left;font-size:10px;text-transform:uppercase}td{padding:4px 8px;border-bottom:1px solid #e0e0e0}tr:nth-child(even)td{background:#f9f9f9}.st{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#333;margin:8px 0 4px;border-bottom:1px solid #ccc;padding-bottom:3px}@media print{body{margin:10px}}</style>
  </head><body>
  ...
  <div class="st">EQUIPE</div><table><thead><tr><th>Cargo</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${(s.equipe||[]).map(e=>`<tr><td>${e.cargo}</td><td>${e.qtd} UN</td><td></td><td></td></tr>`).join('')}</tbody></table>
  <div class="st">GELO</div><table><thead><tr><th>Produto</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${tr('GELO CUBO 4KG',s.geloCubo+' PCT')}${s.geloTrans>0?tr('GELO TRANSLÚCIDO',s.geloTrans+' UN'):''}</tbody></table>
  <div class="st">BEBIDAS SEM ÁLCOOL</div><table><thead><tr><th>Produto</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${tr('ÁGUA TÔNICA',s.aguaTonica+' UN')}${tr('ÁGUA COM GÁS',s.aguaGas+' UN')}</tbody></table>
  ${s.bebidasAlc?`<div class="st">BEBIDAS ALCOÓLICAS</div><table><thead><tr><th>Produto</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${s.bebidasAlc.split('\n').filter(Boolean).map(l=>`<tr><td colspan="2">${l}</td><td></td><td></td></tr>`).join('')}</tbody></table>`:''}
  <div class="st">MATERIAL</div><table><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${Object.entries(s.material||{}).map(([k,v])=>tr(k,v+' UN')).join('')}</tbody></table>
  <div class="st">DESCARTÁVEIS</div><table><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${Object.entries(s.descartaveis||{}).map(([k,v])=>tr(k,v+' UN')).join('')}</tbody></table>
  <div class="st">KIT EQUIPE</div><table><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${Object.entries(s.kitEquipe||{}).map(([k,v])=>tr(k,v+' UN')).join('')}</tbody></table>
  <div class="st">PRODUÇÃO</div><table><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${tr('SUCO DE LIMÃO',s.sucoLimao+' LT')}${tr('XAROPE DE AÇÚCAR',s.xaropeAcucar+' LT')}${tr('MIX FRUTAS VERMELHAS',s.frutasVerm+' UN')}</tbody></table>
  ${s.mixArtesanal?`<div class="st">MIX ARTESANAL</div><table><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${s.mixArtesanal.split('\n').filter(Boolean).map(l=>`<tr><td colspan="2">${l}</td><td></td><td></td></tr>`).join('')}</tbody></table>`:''}
  ${s.xaropesEsp?`<div class="st">XAROPES ESPECIAIS</div><table><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${s.xaropesEsp.split('\n').filter(Boolean).map(l=>`<tr><td colspan="2">${l}</td><td></td><td></td></tr>`).join('')}</tbody></table>`:''}
  ${s.frutas?`<div class="st">FRUTAS / HORTIFRUTI</div><table><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${s.frutas.split('\n').filter(Boolean).map(l=>`<tr><td colspan="2">${l}</td><td></td><td></td></tr>`).join('')}</tbody></table>`:''}
