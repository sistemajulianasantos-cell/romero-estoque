...
const ACESSO = {
  admin:       ['estoque','contratos','agenda','financeiro','analise'],
  financeiro:  ['contratos','agenda','financeiro'],
  operacional: ['contratos','agenda']
};
...
function rAnalise() {
  ...
  // Financeiro — parcelas ligadas ao período
  ...
  const totalEquipe = contratos.reduce((s,c)=>s+(c.equipeTotal||0),0);
  const ticket     = contratos.length ? faturamento / contratos.length : 0;

  // Viagens — transporte por conta da Romero
  const viagens = contratos.filter(c => /romero/i.test(c.transporte||''));

  // Atualizar cards
  const set = (id, v) => { const el=document.getElementById(id); if(el) el.textContent=v; };
  set('an-faturamento', fR(faturamento));
  set('an-recebido',    fR(recebido));
  set('an-a-receber',   fR(aReceber));
  set('an-eventos',     contratos.length.toString());
  set('an-convidados',  totalConv.toLocaleString('pt-BR'));
  set('an-ticket',      ticket ? fR(ticket) : '—');
  set('an-equipe',      totalEquipe.toString() || '—');
  set('an-viagem',      viagens.length.toString());

  // Tabela de viagens
  const secViagem = document.getElementById('an-sec-viagem');
  const tbody = document.getElementById('an-viagem-body');
  if (secViagem && tbody) {
    secViagem.style.display = viagens.length ? '' : 'none';
    tbody.innerHTML = viagens.map(c => `
      <tr style="border-bottom:1px solid #1E2333">
        <td style="padding:8px 12px;color:#E8EAF0">${c.nome||'—'}</td>
        <td style="padding:8px 12px;color:#E8EAF0">${c.tipo||'—'}</td>
        <td style="padding:8px 12px;color:#E8EAF0">${fd(c.data)}</td>
        <td style="padding:8px 12px;color:#8B91A8;font-size:12px">${c.local||'—'}</td>
        <td style="padding:8px 12px;color:#E8EAF0">${c.convidados||'—'}</td>
        <td style="padding:8px 12px;color:#4F8EF7">${c.opcao ? 'R$ '+c.opcao : '—'}</td>
      </tr>`).join('');
  }

  // Distribuição por tipo
  const tiposEl = document.getElementById('an-tipos');
  if (tiposEl) {
    const tipos = {};
    contratos.forEach(c => { const t = c.tipo||'Outro'; tipos[t] = (tipos[t]||0)+1; });
    const cores = ['#4F8EF7','#F7C84F','#4FC78E','#F74F6B','#A04FF7','#F7904F'];
    tiposEl.innerHTML = Object.entries(tipos)
      .sort((a,b)=>b[1]-a[1])
      .map(([tipo,qtd],i) => `
        <div style="background:#1A1F2E;border:1px solid #2A2F42;border-radius:8px;padding:12px 18px;min-width:140px">
          <div style="font-size:22px;font-weight:700;color:${cores[i%cores.length]}">${qtd}</div>
          <div style="font-size:12px;color:#8B91A8;margin-top:2px">${tipo}</div>
        </div>`).join('') || '<span style="color:#8B91A8">Nenhum evento no período.</span>';
  }

  // Datas com menor volume — mostra dias do mês com 0 ou poucos eventos
  const datasEl = document.getElementById('an-datas-livres');
  if (datasEl && mes) {
    ...
    for (let d=1; d<=diasNoMes; d++) {
      if (!eventosPorDia[d]) semEvento.push(d);
    }
    datasEl.innerHTML = semEvento.length
      ? `<span style="color:#8B91A8">Dias sem eventos: </span>` +
        semEvento.map(d=>`<span style="background:#1A1F2E;border:1px solid #2A2F42;border-radius:4px;padding:2px 8px;margin:2px;display:inline-block;font-size:12px;color:#E8EAF0">${String(d).padStart(2,'0')}/${mes}</span>`).join('')
      : '<span style="color:#4FC78E">✓ Todos os dias do mês têm pelo menos 1 evento.</span>';
  } else if (datasEl && !mes) {
    datasEl.innerHTML = '<span style="color:#8B91A8">Selecione um mês para ver os dias disponíveis.</span>';
  }
}
...
function rContratos() {
  ...
  if (!tbody) return;
  if (!lista.length) {
    tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:#8B91A8;padding:24px">Nenhum contrato cadastrado</td></tr>';
    return;
  }
