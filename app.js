...
function rAgenda() {
  ...
  const CIDADES_VIAGEM_AG = ['nova lima','contagem','betim','ribeirão das neves','santa luzia',
    'vespasiano','lagoa santa','pedro leopoldo','sete lagoas','ibirité','esmeraldas','brumadinho',
    'itabirito','ouro preto','mariana','congonhas','conselheiro lafaiete','juiz de fora',
    'uberlândia','uberaba','montes claros','governador valadares','ipatinga','divinópolis',
    'pará de minas','formiga','lavras','alfenas','poços de caldas','três pontas','varginha'];

  const ehViagem = ev => CIDADES_VIAGEM_AG.some(ci => (ev.local||'').toLowerCase().includes(ci))
                      || /por conta da romero/i.test(ev.transporte||'');
