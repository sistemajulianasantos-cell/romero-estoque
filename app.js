...
function extrairDadosContratoRomero(txt) {
  ...
  for (let i = 0; i < linhas.length; i++) {
    ...
    for (const linha of candidatos) {
      if (/portadora|portador|doravante/i.test(linha) && linha.length > 10) {
        ...
        if (nome && !/CONTRATO/i.test(nome) && nome.length > 3) {
  ...
  const mBloco = txt.match(/equipe de trabalho que ser[aá] composta por:([\s\S]*?)(?=\nm\)|\nl\)|\nBebidas|CLÁUSULA)/i);
  if (mBloco) {
    // Cada linha do bloco: "01 (um) Head Bartender" → quantidade + cargo
    const eqRe = /(\d+)\s+\([^)]+\)\s+([^\n\r]+)/g;
    let em;
    while ((em = eqRe.exec(mBloco[1])) !== null) {
      const cargo = em[2].trim().replace(/s\s*$/i, '').trim();
      if (cargo.length > 1) d.equipe.push({ qtd: parseInt(em[1]), cargo });
    }
  }
