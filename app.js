...
function extrairDadosContratoRomero(txt) {
  // Normaliza espaços entre dígitos (PDF às vezes separa "2 5 0" → "250")
  const t = txt.replace(/(\d)\s+(\d)/g,'$1$2').replace(/(\d)\s+(\d)/g,'$1$2');
  const d = {};

  // ── CLIENTE ────────────────────────────────────────────────────────────────
  // Pega a primeira linha que contém "portadora/portador/doravante"
  // e extrai só o nome (antes da vírgula), sem sufixos empresariais
  const linhas = txt.split('\n').map(l=>l.trim()).filter(Boolean);
  for (const linha of linhas) {
    if (/portadora|portador|doravante/i.test(linha) && linha.length > 10) {
      // Pegar tudo antes de ", portadora/portador/CNPJ/CPF/residente/doravante"
      // Mantém nome completo — inclusive empresas (ex: Firenze Empresa de Participações Simples LTDA)
      const nome = linha.split(/,\s*(?:portadora|portador|CNPJ|CPF|residente|doravante)/i)[0].trim();
      if (nome && !/CONTRATO/i.test(nome)) { d.contratante = nome; break; }
    }
  }
  ...
  // ── TIPO DE EVENTO ─────────────────────────────────────────────────────────
  const mTipo = t.match(/\bno\s+(Casamento|Aniversário|Aniversario|Formatura|15\s*Anos|Debutante)\b/i)
             || t.match(/\bem\s+um\s+([\w\s]+?),\s+que\s+será/i)
             || t.match(/\b(Casamento|Aniversário|Aniversario|Formatura|15\s*Anos|Festa\s+\w+)\b/i);
  ...
  if (mD) {
    ...
    const mDE = t.match(/no dia\s+(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})/i);
    if (mDE) {
  ...
  // ── LOCAL ──────────────────────────────────────────────────────────────────
  // Padrão 1: "no endereço localizado no/na X" — até \n, Início, Cerimonia ou CLÁUSULA
  // Padrão 1: "no endereço localizado no/na X" — até \n, Início, Cerimonia ou CLÁUSULA
  const mL1 = txt.match(/endereço localizado (?:no|na)\s+(.+?)(?=\n|Início|Cerimoni|CLÁUSULA)/i);
  if (mL1) {
    d.local = mL1[1].trim().replace(/\s+/g,' ').replace(/\.\s*$/,'');
  } else {
    // Padrão 2: "no Espaço/Fazenda/etc X, localizado Endereço completo"
    // Pega nome do espaço + endereço completo até quebra de linha
    const mL2 = txt.match(/no\s+((?:Espaço|Fazenda|Chácara|Clube|Hotel|Sítio)\s+[^\n]+?),\s*localizado\s+(.+?)(?=\n|Cerimoni|CLÁUSULA)/i);
    if (mL2) {
      d.local = `${mL2[1].trim()}, ${mL2[2].trim().replace(/\.\s*$/,'')}`;
    } else {
      // Padrão 3: qualquer local após "no dia X, no Y" — pega até fim da linha
      const mL3 = txt.match(/realiz\.?\w*\s+no(?:\s+endereço)?\s+(.+?)(?=\n|Cerimoni|CLÁUSULA)/i);
      if (mL3) d.local = mL3[1].trim().replace(/\.\s*$/,'');
    }
  }

  // ── HORÁRIOS — SEMPRE da Cláusula 2b ──────────────────────────────────────
  // Padrão fixo: "sendo o início às HHhHH/HH:HH do dia ... e término às HHhHH"
  const mH = t.match(/sendo o início às\s+(\d{1,2})[Hh:](\d{2})\w*\s+do dia[\s\S]{1,30}?e\s+término às\s+(\d{1,2})[Hh:](\d{2})/i);
  if (mH) {
    d.hrInicio = `${mH[1].padStart(2,'0')}:${mH[2]}`;
    d.hrFim    = `${mH[3].padStart(2,'0')}:${mH[4]}`;
  }

  // ── DURAÇÃO — da cláusula i): "duração de X horas/h de festa" ─────────────
  const mDur = t.match(/dura[cç][aã]o de\s+(\d+)h?\s*(?:horas?\s*)?(?:de\s*festa)?/i);
  ...
  // ── HORA EXTRA valor ───────────────────────────────────────────────────────
  const mHE = t.match(/horas?\s+extras?,?\s+no valor de\s+R\$([\d\.]+,\d{2})/i);
  if (mHE) d.horaExtra = mHE[1];

  // ── EQUIPE — bloco após "equipe de trabalho que será composta por:" ─────────
  // Delimitado até a próxima letra de parágrafo ou "Bebidas" ou "CLÁUSULA"
  d.equipe = [];
  const mBloco = txt.match(/equipe de trabalho que será composta por:\s*\n([\s\S]*?)(?=\n\s*[a-z]\)|\n\s*[lm]\)|\nBebidas|\nCLÁUSULA)/i);
  if (mBloco) {
    const eqRe = /(\d+)\s*\([^)]+\)\s*(Head\s*Bartenders?|Bartenders?|Bar\s*Back|Copeiros?|Garç[oõ]ns?|Auxiliares?|Recepcionistas?)/gi;
    let em;
    while ((em = eqRe.exec(mBloco[1])) !== null) {
      d.equipe.push({ qtd: parseInt(em[1]), cargo: em[2].replace(/s$/i,'').trim() });
    }
  }
  // Ordenar: Head Bartender, Bartender, Bar Back, Copeiro
  const ordemCargo = ['Head Bartender','Bartender','Bar Back','Copeiro','Garçom','Auxiliar','Recepcionista'];
  d.equipe.sort((a,b) => {
    const ia = ordemCargo.findIndex(c => a.cargo.toLowerCase().includes(c.toLowerCase().replace(/s$/,'')));
    const ib = ordemCargo.findIndex(c => b.cargo.toLowerCase().includes(c.toLowerCase().replace(/s$/,'')));
  ...
  // ── VALOR TOTAL ────────────────────────────────────────────────────────────
  const mVT = t.match(/[Vv]alor total do serviço\s*[–\-]\s*R\$\s*([\d\.]+,\d{2})/i);
  if (mVT) d.valorTotal = mVT[1];

  // ── PARCELAS — padrão fixo, 2 linhas específicas ──────────────────────────
  d.parcelas = [];
  // Sinal: "R$X,00 (...) no ato da assinatura deste contrato"
  const mSinal = t.match(/R\$([\d\.]+,\d{2})\s*\([^)]+\)\s*no ato da assinatura deste contrato/i);
  if (mSinal) {
    d.parcelas.push({
      valor: mSinal[1],
  ...
  // Restante: "R$X,00 (...) para o dia DD/MM/AAAA"
  const mRest = t.match(/R\$([\d\.]+,\d{2})\s*\([^)]+\)\s*para o dia\s+(\d{2})[\/\-](\d{2})[\/\-](\d{4})/i);
  if (mRest) {
    d.parcelas.push({
      valor: mRest[1],
  ...
  // ── CARDÁPIO ───────────────────────────────────────────────────────────────
  // Bloco entre "Coquetéis:" e a próxima letra de parágrafo ou CLÁUSULA
  const mCard = txt.match(/(?:Recpetivo|Receptivo|Coquetéis?:)([\s\S]*?)(?=\nf\)|\ng\)|\nCLÁUSULA)/i);
  if (mCard) d.cardapio = ('Cardápio:\n' + mCard[0]).replace(/\s{3,}/g,'\n').trim();
  ...
  const cidadesForaBH = ['nova lima','contagem','betim','ribeirão das neves','santa luzia',
    'sabará','ibirité','vespasiano','lagoa santa','pedro leopoldo','confins','matozinhos',
    'esmeraldas','brumadinho','itabirito','ouro preto','mariana','congonhas','mateus leme',
    'itaúna','divinópolis','sete lagoas'];
  d.transporte = cidadesForaBH.some(c=>(d.local||'').toLowerCase().includes(c))
...
function preencherFormImportacao(txt, nomeArquivo) {
  window._dadosImportacao = extrairDadosContratoRomero(txt);
  ...
  if (resumo) {
    resumo.innerHTML = `
      <div style="display:flex;flex-wrap:wrap;gap:6px;font-size:12px">
        ...
        ${d.transporte?`<span class="tag tag-yellow">🚗 ${d.transporte}</span>`:''}
        ${d.parcelas.length?`<span class="tag tag-green">💰 ${d.parcelas.length} parcela(s) encontrada(s)</span>`:''}
        ${d.horaExtra?`<span class="tag tag-yellow">⚡ Hora extra: R$${d.horaExtra}</span>`:''}
      </div>`;
...
function salvarImportacao() {
  ...
  const contrato = {
    ...
    status:     document.getElementById('imp-status')?.value||'concluido',
    origem: 'importado'
  };
...
function extrairDadosContrato(txt) {
  ...
  if (mData) {
    const ano = mData[3].length === 2 ? '20'+mData[3] : mData[3];
    dadosContrato.data = `${ano}-${mData[2].padStart(2,'0')}-${mData[1].padStart(2,'0')}`;
  }
  ...
  const mOp1 = txt.match(/opção\s*1[:\s]*R\$\s*([\d\.,]+)/i) || txt.match(/R\$\s*([\d\.]+,[\d]+)/);
  if (mOp1) dadosContrato.opcao1 = mOp1[1];
  const mEq = txt.match(/(\d+)\s*(colaboradores?|bartenders?)/i);
  ...
  set('c-opcao', dadosContrato.opcao1);
...
async function gerarContratoDocx() {
  ...
  try {
    ...
    salvarContrato({ nome, cpf, tipo, data, local, convidados:conv, hrIni, hrFim, equipe:dadosContrato.equipe, opcao, valorNum, parc1, parc2, status:'ativo', origem:'sistema' });
...
function salvarContrato(c) {
  salvarContratoCompleto({...c, equipe:[], equipeTexto:c.equipe||'', equipeTotal:0,
    parcelas:[], nomeEvento:c.nome, duracao:'', transporte:''});
}
...
function registrarFichaProducao(c) {
  ...
  D.producoes.push({
    id: 'PRD'+Date.now(),
    evento: c.nomeEvento||c.nome, cliente: c.nome,
    data: c.data, convidados: c.convidados,
    local: c.local, transporte: c.transporte||'',
    hrInicio: c.hrInicio, hrFim: c.hrFim, duracao: c.duracao,
    equipe: c.equipe||[], equipeTexto: c.equipeTexto||'',
    horaExtra: c.horaExtra||'', cardapio: c.cardapio||'',
...
function rContratos() {
  ...
  if (!tbody) return;
  if (!lista.length) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:#8B91A8;padding:24px">Nenhum contrato cadastrado</td></tr>';
    return;
  }
  ...
  [...lista].sort((a,b) => (b.data||'').localeCompare(a.data||'')).forEach(c => {
    ...
    tr.innerHTML = `
      ...
      <td><button class="btn-sm btn-red" onclick="excluirContrato('${c.id}')">Excluir</button></td>
    `;
...
function rAgenda() {
  ...
  if (!lista.length) {
    ...
    document.getElementById('ag-total-colab').innerHTML = '—';
    return;
  }

  // Totalizador por cargo
  const totCargos = {};
  lista.forEach(ev => {
    (ev.equipe||[]).forEach(e => {
    ...
    // fallback se só tem equipeTotal
  ...
  document.getElementById('ag-total-colab').innerHTML = `${totalGeral} total &nbsp;·&nbsp; ${totStr}`;
  ...
  lista.forEach(ev => {
    ...
    tr.innerHTML = `
      ...
      <td>${ev.hrInicio||'—'}</td>
      <td>${ev.hrFim||'—'}</td>
      <td>${ev.duracao||calcDuracao(ev.hrInicio,ev.hrFim)||'—'}</td>
      <td style="font-size:11px;line-height:1.6">${equipeDisplay}</td>
    `;
    tbody.appendChild(tr);
  });
}
...
function setProdView(v) {
  ['lista','manual'].forEach(k => {
    ...
    if (el) el.style.display = v === k ? '' : 'none';
    const btn = document.getElementById('pv-'+k);
    if (btn) btn.classList.toggle('active', v === k);
  });
  ...
  [...lista].sort((a,b)=>(b.data||'').localeCompare(a.data||'')).forEach(p => {
    ...
    div.innerHTML = `
      <div class="sec-head" style="display:flex;align-items:center;gap:10px">
        ...
        <button class="btn-sm btn-red" style="margin-left:auto" onclick="excluirProducao('${p.id}')">Excluir</button>
      </div>
