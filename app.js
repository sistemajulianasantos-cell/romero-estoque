...
function extrairDadosContratoRomero(txt) {
  // Normaliza espaços entre dígitos (PDF separa "2 5 0" → "250")
  const t = txt.replace(/(\d)\s+(\d)/g,'$1$2').replace(/(\d)\s+(\d)/g,'$1$2');
  const d = {};

  // CONTRATANTE
  const mCont = t.match(/^([A-ZÀ-Úa-zà-ú][^\n]{5,80}?),\s*portadora|^([A-ZÀ-Úa-zà-ú][^\n]{5,80}?),\s*portador/m);
  if (mCont) d.contratante = (mCont[1]||mCont[2]).trim();
  ...
  const mEnd = t.match(/residente\s+n[ao]\s+([^\n]+?),?\s*doravante/i);
  if (mEnd) d.endereco = mEnd[1].trim();

  // TIPO DE EVENTO
  const mTipo = t.match(/\bno\s+(Casamento|Aniversário|Aniversario|Formatura|15\s*Anos|Debutante)\b/i)
             || t.match(/\b(Casamento|Aniversário|Aniversario|Formatura|15\s*Anos)\b/i);
  ...
  // DATA
  const mData = t.match(/no dia\s+(\d{2})[\/\-](\d{2})[\/\-](\d{4})/i)
             || t.match(/(\d{2})[\/\-](\d{2})[\/\-](\d{4})/);
  if (mData) {
    const partes = mData[0].match(/(\d{2})[\/\-](\d{2})[\/\-](\d{4})/);
    d.data = `${partes[3]}-${partes[2].padStart(2,'0')}-${partes[1].padStart(2,'0')}`;
  }

  // CONVIDADOS
  const mConv = t.match(/atender\s+(\d+)\s*convidados/i)
             || t.match(/qual\s+seja\s+(\d+)\s*pessoas/i)
             || t.match(/(\d+)\s*convidados/i);
  if (mConv) d.convidados = mConv[1];

  // LOCAL
  const mLocal = t.match(/endereço localizado no\s+(.+?)(?:\n|Início)/i)
              || t.match(/no endereço\s+(.+?)(?:\n|Início)/i);
  if (mLocal) d.local = mLocal[1].trim().replace(/\s+/g,' ');

  // HORÁRIO INÍCIO
  const mIni = t.match(/[Ii]nício\s*(?:da\s*festa)?[:\s]+(\d{1,2})h(\d{2})/i)
            || t.match(/início\s*às\s*(\d{1,2})h(\d{2})/i);
  if (mIni) d.hrInicio = `${mIni[1].padStart(2,'0')}:${mIni[2]}`;

  // HORÁRIO FIM
  const mFim = t.match(/término\s*às\s*(\d{1,2})h(\d{2})/i)
            || t.match(/até\s+(\d{1,2})h(\d{2})/i);
  if (mFim) d.hrFim = `${mFim[1].padStart(2,'0')}:${mFim[2]}`;

  // DURAÇÃO
  const mDur = t.match(/duração de\s*(\d+)\s*horas/i);
  if (mDur) { d.duracao = mDur[1] + 'h'; }
  ...
  // EQUIPE DETALHADA
  d.equipe = [];
  const eqRe = /(\d+)\s*\([^)]+\)\s*(Head\s*Bartender|Bartenders?|Bar\s*Back|Copeiros?|Garç[oõ]ns?|Auxiliares?|Recepcionistas?|Coordenadores?)/gi;
  let em;
  while ((em = eqRe.exec(t)) !== null) {
    d.equipe.push({ qtd: parseInt(em[1]), cargo: em[2].trim() });
  }
  ...
  const mVT = t.match(/[Vv]alor total do serviço\s*[–\-]\s*R\$\s*([\d\.]+,\d{2})/i)
           || t.match(/[Vv]alor total\s*[–\-]\s*R\$\s*([\d\.]+,\d{2})/i);
  if (mVT) d.valorTotal = mVT[1];

  // PARCELAS COM DATAS REAIS
  d.parcelas = [];
  const prRe = /R\$([\d\.]+,\d{2})\s*\([^)]+\)\s*([^\n\.]{5,80})/gi;
  let pm;
  while ((pm = prRe.exec(t)) !== null) {
    const desc = pm[2].trim();
    const val  = pm[1];
    if (/assinatura|ato|contrato/i.test(desc) && !/convidado|hora extra/i.test(desc)) {
      d.parcelas.push({ valor: val, descricao: 'Sinal — assinatura do contrato', vencimento: new Date().toISOString().slice(0,10) });
    } else {
      const mD = desc.match(/(\d{2})[\/\-](\d{2})[\/\-](\d{4})/);
      if (mD) {
        const venc = `${mD[3]}-${mD[2]}-${mD[1]}`;
        d.parcelas.push({ valor: val, descricao: `Restante — venc. ${mD[0]}`, vencimento: venc });
      }
    }
  }

  // HORA EXTRA
  const mHE = t.match(/valor de R\$([\d\.]+,\d{2})\s*(?:cada|por)\s*hora extra/i);
  if (mHE) d.horaExtra = mHE[1];

  // CONVIDADO EXTRA
  const mCE = t.match(/R\$([\d\.]+,\d{2})\s*\([^)]+\)\s*,?\s*por convidado extra/i);
  if (mCE) d.convExtra = mCE[1];

  // CARDÁPIO
  const mCardapio = t.match(/Recpetivo|Receptivo([\s\S]*?)(?=\nf\)|\ng\)|\nCLÁUSULA)/i);
  if (mCardapio) d.cardapio = mCardapio[0].replace(/\s+/g,' ').trim();

  // BEBIDAS ROMERO
  const mBeb = t.match(/Bebidas fornecidas pela ROMERO[^:]*?sendo:\s*([^\n]+)/i);
  if (mBeb) d.bebidasRomero = mBeb[1].trim();

  // TRANSPORTE (60km de BH)
  const cidadesRomero = ['nova lima','contagem','betim','ribeirão das neves','santa luzia',
    'sabará','ibirité','vespasiano','lagoa santa','pedro leopoldo','confins','matozinhos',
    'esmeraldas','brumadinho','itabirito','ouro preto','mariana','congonhas'];
  d.transporte = cidadesRomero.some(c=>(d.local||'').toLowerCase().includes(c))
