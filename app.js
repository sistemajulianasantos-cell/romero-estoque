...
function extrairDadosContratoRomero(txt) {
  ...
  // Se contém "CONSIGNADA" → campo bebidasConsignadas; senão → bebidasRomero
  {
    const mMhdr = t.match(/\bm\)[^\n]*Bebidas fornecidas pela ROMERO([^\n]*)/i);
    const isConsignada2 = mMhdr ? /CONSIGNADA/i.test(mMhdr[1]) : false;
    const mM = t.match(/\bm\)[^\n]*Bebidas fornecidas pela ROMERO[^\n]*\n([\s\S]*?)(?=(?:CLÁUSULA|\bn\)|\bo\)|Assinatura)|$)/i);
    console.log("[DEBUG mM]", mM ? mM[1].substring(0,100) : "NULL", "isConsignada:", isConsignada2);
    if (mM) {
