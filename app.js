...
// ── Parser do texto da folha de produção ────────────────
function parsearTextoProducao(txt) {
  ...
  // Cardápio: pega tudo entre CARDÁPIO e próxima seção em caixa alta
  const cardRe = /CARD[ÁA]PIO[:\s*]*([\s\S]*?)(?=\n[A-Z\s]{4,}:|\n[A-Z\s]{4,}\n|$)/i;
  const cardM = txt.match(cardRe);
  const cardapio = cardM ? cardM[1].replace(/\*+/g,'').trim() : '';

  // Bebidas consignadas
  const consRe = /BEBIDAS[\s]+CONSIGNADA[S]?[:\s*]*([\s\S]*?)(?=\nBEBIDAS[\s]+|\nCARD|\n[A-Z]{4,}:|$)/i;
  const consM = txt.match(consRe);
  const bebidasConsignadas = consM ? consM[1].replace(/\*+/g,'').trim() : '';

  // Bebidas cliente
  const cliRe = /BEBIDAS[\s]+CLIENTE[:\s*]*([\s\S]*?)(?=\nCARD|\nBEBIDAS[\s]+ROMERO|\n[A-Z]{4,}:|$)/i;
  const cliM = txt.match(cliRe);
  const bebidasCliente = cliM ? cliM[1].replace(/\*+/g,'').trim() : '';

  // Bebidas Romero
  const romRe = /BEBIDAS[\s]+ROMERO[:\s*]*([\s\S]*?)(?=\nCARD|\n[A-Z]{4,}:|$)/i;
  const romM = txt.match(romRe);
  const bebidasRomero = romM ? romM[1].replace(/\*+/g,'').trim() : '';

  // Equipe — linhas com cargo e quantidade
  const equipe = [];
  ...
  // Horário
  const horRe = /HORARIO[\s]+FESTA[:\s*]+([\d:]+)[\s]+[AÀ]+S[\s]+([\d:]+)/i;
  ...
  return {
    ...
    nome:              get('CLIENTE'),
    data:              '',
    convidados:        convM ? convM[1] : '',
    local:             get('LOCAL'),
    transporte:        get('TRANSPORTE'),
    pecas:             get('PE[ÇC]AS'),
    cerimonial:        get('CERIMONIAL'),
    decorador:         get('DECORADOR'),
    buffet:            get('BUFFET'),
    layout:            get('LAYOUT'),
    pontoEnergia:      get('PONTO DE ENERGIA'),
    hrChegadaEquipe:   get('HORARIO DE CHEGADA DA EQUIPE'),
    hrInicio:          horM ? horM[1] : '',
    hrFim:             horM ? horM[2] : '',
    duracao:           durM ? durM[1] : '',
    horaExtra:         heM ? heM[1] : '',
    equipe:            equipe,
    ...
    cardapio:          cardapio,
    bebidasConsignadas: bebidasConsignadas,
    bebidasCliente:    bebidasCliente,
    bebidasRomero:     bebidasRomero,
    obs:               '',
  };
}
    ...
    <div class="sec">
      ...
      <div id="prod-campos-extraidos" style="display:none;padding:0 16px 16px">
        ...
        <div style="display:flex;gap:8px;margin-top:14px">
          <button class="btn" onclick="salvarProducaoForm()" style="background:var(--green)">💾 Salvar Produção</button>
...
function lerTextoProducao() {
  ...
  set('pf-proposta', d.proposta);
  set('pf-tipo', d.tipo);
  set('pf-nome', d.nome);
  set('pf-data', d.data);
  set('pf-convidados', d.convidados);
  set('pf-local', d.local);
  if (d.transporte) {
    const sel = document.getElementById('pf-transporte');
    if (sel) sel.value = d.transporte.includes('Romero') ? 'Por conta da Romero' : 'Por conta do colaborador';
  }
  set('pf-pecas', d.pecas);
  set('pf-cerimonial', d.cerimonial);
  set('pf-decorador', d.decorador);
  set('pf-buffet', d.buffet);
  set('pf-layout', d.layout);
  if (d.pontoEnergia) {
    const sel = document.getElementById('pf-pontoenergia');
    if (sel) sel.value = d.pontoEnergia.toUpperCase().startsWith('S') ? 'SIM' : 'NAO';
  }
  set('pf-chegada', d.hrChegadaEquipe);
  set('pf-hrini', d.hrInicio);
  set('pf-hrfim', d.hrFim);
  set('pf-duracao', d.duracao);
  set('pf-horaextra', d.horaExtra);
  set('pf-equipe', d.equipe.map(e => e.qtd + ' ' + e.cargo).join('\n'));
  set('pf-cons', d.bebidasConsignadas);
  set('pf-cli', d.bebidasCliente);
  set('pf-romero', d.bebidasRomero);
  set('pf-cardapio', d.cardapio);
  set('pf-obs', d.obs);

  const campos = document.getElementById('prod-campos-extraidos');
...
function salvarProducaoForm() {
  ...
  // Parsear equipe do textarea
  const equipeLinhas = get('pf-equipe').split('\n').filter(Boolean);
  const equipe = equipeLinhas.map(l => {
    const m = l.match(/^(\d+)\s+(.+)/);
    return m ? { qtd: parseInt(m[1]), cargo: m[2].trim().toUpperCase() } : null;
  }).filter(Boolean);

  if (!D.producoes) D.producoes = [];
  D.producoes.push({
    id:              'PRD' + Date.now(),
    proposta:        get('pf-proposta'),
    evento:          evento,
    nomeEvento:      evento,
    tipo:            get('pf-tipo'),
    nome:            get('pf-nome'),
    cliente:         get('pf-nome'),
    data:            data,
    convidados:      get('pf-convidados'),
    local:           get('pf-local'),
    transporte:      get('pf-transporte'),
    pecas:           get('pf-pecas'),
    cerimonial:      get('pf-cerimonial'),
    decorador:       get('pf-decorador'),
    buffet:          get('pf-buffet'),
    layout:          get('pf-layout'),
    pontoEnergia:    get('pf-pontoenergia'),
    hrChegadaEquipe: get('pf-chegada'),
    hrInicio:        get('pf-hrini'),
    hrFim:           get('pf-hrfim'),
    duracao:         get('pf-duracao'),
    horaExtra:       get('pf-horaextra'),
    equipe:          equipe,
    equipeTexto:     equipe.map(e => e.qtd + ' ' + e.cargo).join(' · '),
    equipeTotal:     equipe.reduce((s,e) => s + e.qtd, 0),
    bebidasConsignadas: get('pf-cons'),
    bebidasCliente:  get('pf-cli'),
    bebidasRomero:   get('pf-romero'),
    cardapio:        get('pf-cardapio'),
    obs:             get('pf-obs'),
    criadoEm:        new Date().toISOString()
  });
  sv('producoes');
...
function rProducoes() {
  ...
  if (!lista.length) {
    cont.innerHTML = '<div style="text-align:center;color:var(--text3);padding:32px;font-size:13px">Nenhuma produção registrada. Clique em "+ Nova Produção".</div>';
    return;
  }
    ...
    div.innerHTML = `
      <div class="sec-head" style="display:flex;align-items:center;gap:10px">
        ...
        <div style="margin-left:auto;display:flex;gap:6px">
          <button class="btn-sm" style="background:#6C63FF" onclick="gerarFolhaProducao('${p.id}')">📄 Word</button>
      ...
      <div style="padding:10px 16px;display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px;font-size:12px">
        ...
        ${p.obs ? `<div style="grid-column:1/-1"><span style="color:var(--text3)">OBS:</span> ${p.obs}</div>` : ''}
      </div>
    `;
...
function editarProducaoModal(id) {
  ...
  if (!p) return;
  // Preenche o form de nova producao com os dados existentes e salva substituindo
  setProdView('nova');
  setTimeout(() => {
    const set = (elId, val) => { const el = document.getElementById(elId); if (el) el.value = val || ''; };
    document.getElementById('prod-texto-raw').value = '';
    set('pf-proposta', p.proposta);
    set('pf-tipo', p.tipo);
    set('pf-nome', p.nome||p.cliente);
    set('pf-data', p.data);
    set('pf-convidados', p.convidados);
    set('pf-local', p.local);
    const tSel = document.getElementById('pf-transporte');
    if (tSel && p.transporte) tSel.value = p.transporte.includes('Romero') ? 'Por conta da Romero' : 'Por conta do colaborador';
    set('pf-pecas', p.pecas);
    set('pf-cerimonial', p.cerimonial);
    set('pf-decorador', p.decorador);
    set('pf-buffet', p.buffet);
    set('pf-layout', p.layout);
    const peSel = document.getElementById('pf-pontoenergia');
    if (peSel && p.pontoEnergia) peSel.value = p.pontoEnergia;
    set('pf-chegada', p.hrChegadaEquipe);
    set('pf-hrini', p.hrInicio);
    set('pf-hrfim', p.hrFim);
    set('pf-duracao', p.duracao);
    set('pf-horaextra', p.horaExtra);
    set('pf-equipe', (p.equipe||[]).map(e => e.qtd + ' ' + e.cargo).join('\n'));
    set('pf-cons', p.bebidasConsignadas);
    set('pf-cli', p.bebidasCliente);
    set('pf-romero', p.bebidasRomero);
    set('pf-cardapio', p.cardapio);
    set('pf-obs', p.obs);
    document.getElementById('prod-campos-extraidos').style.display = '';
    // Sobrescrever salvar para atualizar em vez de criar novo
    const btn = document.querySelector('#prod-campos-extraidos .btn');
    if (btn) {
      ...
      btn.onclick = () => {
        ...
        D.producoes[idx] = {
          ...D.producoes[idx],
          proposta: get('pf-proposta'), tipo: get('pf-tipo'), nome: get('pf-nome'),
          cliente: get('pf-nome'), evento, nomeEvento: evento,
          ...
          horaExtra: get('pf-horaextra'), equipe, equipeTexto: equipe.map(e => e.qtd+' '+e.cargo).join(' · '),
          equipeTotal: equipe.reduce((s,e) => s+e.qtd, 0),
...
function calcSeparacao(conv, equipe) {
  const bt = (equipe.filter(e => (e.cargo||'').toLowerCase().includes('bartender')).reduce((s,e)=>s+(parseInt(e.qtd)||0),0))
    || Math.max(1, Math.ceil(conv / 30));
  const totalEquipe = equipe.reduce((s,e)=>s+(parseInt(e.qtd)||0),0) || bt;
  return {
    bt, totalEquipe,
    material: {
      'TAPETINHO': bt*2, 'PRATINHO PRETO': bt, 'GARRAFA SOUR': bt, 'GARRAFA SIMPLE': bt,
      'MIXING GLASS': bt, 'BOWL PRETO': bt*2, 'PÁ DE GELO': bt, 'COQUETELEIRA COMPLETA': bt*2,
      'FUNIL': Math.max(1,Math.ceil(bt/3)), 'TÁBUA BRANCA': Math.max(1,Math.ceil(bt/4)),
      'FACA': bt, 'DOSADOR': bt, 'DESCASCADOR': bt, 'PENEIRINHA': bt, 'MINI RALO': bt,
      'BIQUEIRA': bt*2, 'BAILARINA': bt, 'MACERADOR': Math.max(1,Math.ceil(bt/2)),
      'PINÇA': bt, 'PEGADOR DE GELO': bt, 'STREINER': bt,
      'BALDE COPA': Math.max(1,Math.ceil(bt/3)), 'ESCORREDOR': Math.max(1,Math.ceil(bt/3)),
      'DETERGENTE': 1, 'BUCHA': 1, 'COOLER REPOSIÇÃO': Math.max(1,Math.ceil(bt/4)),
      'TÉRMICA PRETA': Math.ceil(bt/2), 'CAIXA AZUL DE GELO': 1,
      'CUBA PRETA': Math.ceil(bt/2), 'LIXEIRA': Math.ceil(bt/2),
      'RÁDIO': totalEquipe, 'TESOURA': Math.max(1,Math.ceil(bt/3)),
      'SUPORTE PARA TÉRMICA': Math.ceil(bt/2),
    },
    descartaveis: {
      'ÁLCOOL': Math.max(1,Math.ceil(bt/3)),
      'GUARDANAPO': Math.max(3,Math.ceil(conv/50)*2),
      'CANUDO': Math.max(2,Math.ceil(conv/50)),
      'SACO DE LIXO': Math.max(3,Math.ceil(conv/40)),
      'COPO DESCARTÁVEL': 1,
      'PANO DE PRATO': Math.max(4,bt),
    },
    kitEquipe: { 'LANCHE': totalEquipe, 'REFEIÇÃO': totalEquipe, 'UNIFORME': totalEquipe },
    geloBase: Math.ceil(conv/5),
    aguaBase: Math.ceil(conv/5),
    prodBase: {
      sucoLimao: Math.ceil(conv/20),
      xarope: Math.ceil(conv/20),
      frutasVerm: Math.ceil(conv/20),
    }
  };
}
...
function salvarSeparacao() {
  ...
  const sep = {
    id: 'SEP'+Date.now(), producaoId: prodId,
    evento: p.evento||p.cliente||'', cliente: p.cliente||'',
    data: p.data||'', convidados: conv, local: p.local||'',
    hrInicio: p.hrInicio||'', hrFim: p.hrFim||'',
    equipe: p.equipe||[], equipeTexto: p.equipeTexto||'',
    bartenders: bt, totalEquipe: teq,
...
function imprimirSeparacao(id) {
  ...
  <div class="st">EQUIPE</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Cargo</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${(s.equipe||[]).map(e=>`<tr><td>${e.cargo}</td><td>${e.qtd} UN</td><td></td><td></td></tr>`).join('')}</tbody></table>
  <div class="st">GELO</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Produto</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${tr('GELO CUBO 4KG',s.geloCubo+' PCT')}${s.geloTrans>0?tr('GELO TRANSLÚCIDO',s.geloTrans+' UN'):''}</tbody></table>
  <div class="st">BEBIDAS SEM ÁLCOOL</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Produto</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${tr('ÁGUA TÔNICA',s.aguaTonica+' UN')}${tr('ÁGUA COM GÁS',s.aguaGas+' UN')}</tbody></table>
  ${s.bebidasAlc?`<div class="st">BEBIDAS ALCOÓLICAS</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Produto</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${s.bebidasAlc.split('\n').filter(Boolean).map(l=>`<tr><td colspan="2">${l}</td><td></td><td></td></tr>`).join('')}</tbody></table>`:''}
  <div class="st">MATERIAL</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${Object.entries(s.material||{}).map(([k,v])=>tr(k,v+' UN')).join('')}</tbody></table>
  <div class="st">DESCARTÁVEIS</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${Object.entries(s.descartaveis||{}).map(([k,v])=>tr(k,v+' UN')).join('')}</tbody></table>
  <div class="st">KIT EQUIPE</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${Object.entries(s.kitEquipe||{}).map(([k,v])=>tr(k,v+' UN')).join('')}</tbody></table>
  <div class="st">PRODUÇÃO</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${tr('SUCO DE LIMÃO',s.sucoLimao+' LT')}${tr('XAROPE DE AÇÚCAR',s.xaropeAcucar+' LT')}${tr('MIX FRUTAS VERMELHAS',s.frutasVerm+' UN')}</tbody></table>
  ${s.mixArtesanal?`<div class="st">MIX ARTESANAL</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${s.mixArtesanal.split('\n').filter(Boolean).map(l=>`<tr><td colspan="2">${l}</td><td></td><td></td></tr>`).join('')}</tbody></table>`:''}
  ${s.xaropesEsp?`<div class="st">XAROPES ESPECIAIS</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${s.xaropesEsp.split('\n').filter(Boolean).map(l=>`<tr><td colspan="2">${l}</td><td></td><td></td></tr>`).join('')}</tbody></table>`:''}
  ${s.frutas?`<div class="st">FRUTAS / HORTIFRUTI</div><table><colgroup><col class='c-item'><col class='c-qtd'><col class='c-saida'><col class='c-volta'></colgroup><thead><tr><th>Item</th><th>Qtd</th><th>Saída</th><th>Volta</th></tr></thead><tbody>${s.frutas.split('\n').filter(Boolean).map(l=>`<tr><td colspan="2">${l}</td><td></td><td></td></tr>`).join('')}</tbody></table>`:''}
