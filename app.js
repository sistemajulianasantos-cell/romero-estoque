...
function preencherFormImportacao(txt, nomeArquivo) {
  // Reutiliza a mesma lógica de extração
  const d = {};
  if (/casamento/i.test(txt)) d.tipo = 'Casamento';
  else if (/15 anos/i.test(txt)) d.tipo = '15 Anos';
  else if (/aniversário|aniversario/i.test(txt)) d.tipo = 'Aniversário';
  else if (/formatura/i.test(txt)) d.tipo = 'Formatura';
  else d.tipo = 'Evento';

  const mData = txt.match(/(\d{1,2})[\/-](\d{1,2})[\/-](\d{2,4})/);
  if (mData) {
    const ano = mData[3].length === 2 ? '20'+mData[3] : mData[3];
    d.data = `${ano}-${mData[2].padStart(2,'0')}-${mData[1].padStart(2,'0')}`;
  }
  const mConv = txt.match(/(\d+)\s*(convidados?|pessoas?|pax)/i);
  if (mConv) d.convidados = mConv[1];

  const mLocal = txt.match(/(?:local|espaço|venue|realização)[:\s]+([^\n,\.]{3,50})/i);
  if (mLocal) d.local = mLocal[1].trim();

  // Tentar pegar nome do cliente do próprio texto
  const mNome = txt.match(/(?:contratante|cliente|nome)[:\s]+([A-ZÀ-Ú][a-zà-ú]+(?:\s+[A-ZÀ-Ú][a-zà-ú]+)+)/);
  if (mNome) d.nome = mNome[1].trim();

  // Valor — pega o maior valor monetário encontrado (geralmente o total)
  const valores = [...txt.matchAll(/R\$\s*([\d\.]+,[\d]{2})/g)].map(m => ({
    str: m[1],
    num: parseFloat(m[1].replace(/\./g,'').replace(',','.'))
  })).sort((a,b) => b.num - a.num);
  if (valores.length) d.valor = valores[0].str;

  const mEq = txt.match(/(\d+)\s*(colaboradores?|bartenders?)/i);
  if (mEq) d.equipe = mEq[1];

  const mIni = txt.match(/(\d{1,2}):(\d{2})\s*(?:h|hs|hora)/i);
  if (mIni) d.hrIni = `${mIni[1].padStart(2,'0')}:${mIni[2]}`;
  ...
  set('imp-ini', d.hrIni);
  set('imp-equipe', d.equipe);
  set('imp-valor', d.valor);
}
...
function salvarImportacao() {
  ...
  const hrIni = document.getElementById('imp-ini')?.value || '';
  const hrFim = document.getElementById('imp-fim')?.value || '';
  ...
  salvarContrato({
    ...
    hrIni, hrFim,
    equipe:     document.getElementById('imp-equipe')?.value?.trim() || '',
    opcao: valorText, valorNum, parc1, parc2,
    status:     document.getElementById('imp-status')?.value || 'concluido',
    origem: 'importado'
  });

  limparImportacao();
  alert('Contrato importado! Agenda e financeiro atualizados.');
...
async function gerarContratoDocx() {
  ...
  try {
    ...
    docx.setData({
      NOME_CONTRATANTE: nome, CPF_CONTRATANTE: cpf, ENDERECO_CONTRATANTE: endereco,
      TIPO_EVENTO: tipo, DATA_EVENTO: dtFormatada || dtSimples, DATA_SIMPLES: dtSimples,
      NUMERO_CONVIDADOS: conv, LOCAL_EVENTO: local,
      HORARIO_INICIO: hrIni, HORARIO_FIM: hrFim,
      DURACAO: (dadosContrato.duracao||'') + 'h',
      VALOR_TOTAL: 'R$ ' + opcao, VALOR_PARCELA1: 'R$ ' + parc1, VALOR_PARCELA2: 'R$ ' + parc2,
      CERIMONIA: cer, EQUIPE: dadosContrato.equipe || '',
...
function salvarContrato(c) {
  ...
  sv('contratos');
  // Alimentar agenda
  registrarEventoNaAgenda({ nome: c.nome, data: c.data, tipo: c.tipo, local: c.local, conv: c.convidados, hrIni: c.hrIni, hrFim: c.hrFim, equipe: c.equipe });
  // Alimentar financeiro (só se tiver valor)
  if (c.valorNum > 0) registrarParcelasFinanceiro({ nome: c.nome, data: c.data, valorNum: c.valorNum, parc1: c.parc1, parc2: c.parc2 });
}
...
function rProducoes() {
  ...
  if (!tbody) return;
  if (!lista.length) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:#8B91A8;padding:24px">Nenhuma produção registrada</td></tr>';
    return;
  }
  tbody.innerHTML = '';
  [...lista].sort((a,b) => (b.data||'').localeCompare(a.data||'')).forEach(p => {
    ...
    tr.innerHTML = `
      <td>${fd(p.data)}</td>
      <td><strong>${p.evento||'—'}</strong></td>
      <td>${p.cliente||'—'}</td>
      <td>${p.convidados||'—'}</td>
      <td style="max-width:300px;white-space:pre-wrap;word-break:break-word">${p.obs||'—'}</td>
      <td><button class="btn-sm btn-red" onclick="excluirProducao('${p.id}')">Excluir</button></td>
    `;
    tbody.appendChild(tr);
  });
}
