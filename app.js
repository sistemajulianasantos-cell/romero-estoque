...
async function gerarFolhaProducao(id) {
  ...
  // Importa docx via CDN se necessário
  if (!window.docx) {
  ...
  const linha = (label, valor) => new Paragraph({
    ...
    children: [
      ...
      new TextRun({ text: valor || '—', font: 'Arial', size: 22 }),
    ]
  });

  const secao = label => new Paragraph({
  ...
  const horaExtra = p.horaExtra
    ? 'R$ ' + parseFloat(p.horaExtra).toLocaleString('pt-BR', { minimumFractionDigits: 2 })
    : '—';

  // Equipe
  const equipeLinhas = [];
  if (Array.isArray(p.equipe) && p.equipe.length) {
    p.equipe.forEach(e => equipeLinhas.push(new Paragraph({
      spacing: { after: 40 },
      children: [new TextRun({ text: String(e.qtd).padStart(2,'0') + ' ' + e.cargo.toUpperCase(), font: 'Arial', size: 22 })]
    })));
  } else if (p.equipeTexto) {
    p.equipeTexto.split('·').forEach(e => equipeLinhas.push(new Paragraph({
      spacing: { after: 40 },
      children: [new TextRun({ text: e.trim().toUpperCase(), font: 'Arial', size: 22 })]
    })));
  }

  // Cardápio
  const cardapioLinhas = (p.cardapio||'').split('\n').filter(Boolean).map(l =>
    new Paragraph({
      spacing: { after: 60 },
      children: [new TextRun({ text: l.trim(), font: 'Arial', size: 22 })]
    })
  );
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        new Paragraph({
          spacing: { after: 40 },
          children: [new TextRun({ text: 'ROMERO COQUETÉIS E EVENTOS', bold: true, font: 'Arial', size: 28, color: 'B8860B' })]
        }),
        new Paragraph({
          spacing: { after: 200 },
          border: { bottom: { style: BorderStyle.SINGLE, size: 6, color: 'B8860B' } },
          children: [new TextRun({ text: 'FOLHA DE PRODUÇÃO', bold: true, font: 'Arial', size: 24 })]
        }),
        linha('PROPOSTA', p.proposta || '—'),
        linha('OCASIÃO', (p.tipo||'').toUpperCase()),
        linha('CLIENTE', (p.cliente||'').toUpperCase()),
        linha('DIA', fmtData(p.data)),
        linha('CONVIDADOS', String(p.convidados||'—')),
        linha('LOCAL', (p.local||'—').toUpperCase()),
        linha('TRANSPORTE', (p.transporte||'POR CONTA DO COLABORADOR').toUpperCase()),
        linha('PEÇAS', (p.pecas||'').toUpperCase() || '—'),
        linha('CERIMONIAL', (p.cerimonial||'').toUpperCase() || '—'),
        linha('DECORADOR', (p.decorador||'').toUpperCase() || '—'),
        linha('BUFFET', (p.buffet||'').toUpperCase() || '—'),
        linha('LAYOUT', (p.layout||'').toUpperCase() || '—'),
        linha('PONTO DE ENERGIA', (p.pontoEnergia||'NÃO').toUpperCase()),
        linha('HORÁRIO CHEGADA DA EQUIPE', p.hrChegadaEquipe || '—'),
        linha('HORÁRIO FESTA', hrFesta),
        linha('HORA EXTRA', horaExtra),
        secao('EQUIPE:'),
        ...equipeLinhas,
        ...(p.bebidasCliente ? [linha('BEBIDAS CLIENTE', (p.bebidasCliente||'').toUpperCase())] : []),
        ...(p.bebidasRomero  ? [linha('BEBIDAS ROMERO',  (p.bebidasRomero||'').toUpperCase())]  : []),
        ...(cardapioLinhas.length ? [secao('CARDÁPIO:'), ...cardapioLinhas] : []),
        ...(p.obs ? [linha('OBS', p.obs)] : []),
      ]
    }]
  });
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = 'PRODUCAO_' + (p.cliente||'evento').replace(/\s+/g,'_').toUpperCase() + '_' + (p.data||'').replace(/-/g,'') + '.docx';
  a.click();
