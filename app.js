...
function rAnalise() {
  ...
  // Viagens — 60km+ de BH
  const CIDADES_VIAGEM = ['nova lima','contagem','betim','ribeirão das neves','santa luzia','vespasiano','lagoa santa','pedro leopoldo','sete lagoas','ibirité','esmeraldas','brumadinho','itabirito','ouro preto','mariana','congonhas','conselheiro lafaiete','juiz de fora','uberlândia','uberaba','montes claros','governador valadares','ipatinga','divinópolis'];
  const viagems = contratos.filter(c => CIDADES_VIAGEM.some(ci => (c.local||'').toLowerCase().includes(ci)) || /por conta da romero/i.test(c.transporte||''));
...
function rContratos() {
  ...
  if (!tbody) return;
  if (!lista.length) {
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:#8B91A8;padding:24px">Nenhum contrato cadastrado</td></tr>';
    return;
  }
  ...
  [...lista].sort((a,b) => (b.data||'').localeCompare(a.data||'')).forEach(c => {
    ...
    tr.innerHTML = `
      ...
      <td style="white-space:nowrap">
        <button class="btn-sm" onclick="editarEquipeContrato('${c.id}')" title="Editar equipe">✏️</button>
...
function editarEquipeContrato(id) {
  const c = (D.contratos||[]).find(x => x.id === id);
  if (!c) return;
  const atual = c.equipeTexto || '';
  const nova = prompt(
    `Equipe do contrato: ${c.nome}\n\nFormato: 01 Head Bartender · 04 Bartender · 01 Bar Back · 01 Copeiro\n\nEquipe atual:`,
    atual
  );
  if (nova === null) return;
  const idx = (D.contratos||[]).findIndex(x => x.id === id);
  if (idx === -1) return;
  // Parsear equipeTexto para array de equipe
  const equipeArray = nova.split('·').map(e => {
    const m = e.trim().match(/^(\d+)\s+(.+)$/);
    return m ? { qtd: parseInt(m[1]), cargo: m[2].trim() } : null;
  }).filter(Boolean);
  D.contratos[idx].equipeTexto  = nova.trim();
  D.contratos[idx].equipe       = equipeArray;
  D.contratos[idx].equipeTotal  = equipeArray.reduce((s,e)=>s+(e.qtd||0),0);
  sv('contratos'); rContratos();
}
