...
function extrairDadosContratoRomero(txt) {
  ...
  // "Responsabilizar-se pelo fornecimento dos destilados... sendo: BEBIDA | ESTIMATIVA"
  {
    const mL = t.match(/l\)\s*Responsabilizar[^\n]*destilados?[^\n]*\n([\s\S]*?)(?=\nm\)|\nCLÁUSULA|\n[A-Z]{4,}\s*[12])/i);
    if (mL) {
      const linhas = mL[1].split('\n').map(l=>l.trim()).filter(l=>l && !/^BEBIDA|^ESTIM|^VALOR/i.test(l));
      d.bebidasCliente = linhas.join('\n').trim();
    }
  }

  // ── BEBIDAS CONSIGNADAS — cláusula 2, letra m) ────────────────────────────
  // "Bebidas fornecidas pela ROMERO COQUETEIS de forma CONSIGNADA, sendo:"
  // Tabela: BEBIDA | ESTIMATIVA | VALOR
  {
    const mM = t.match(/m\)\s*Bebidas fornecidas pela ROMERO[^\n]*CONSIGNADA[^\n]*\n([\s\S]*?)(?=\nn\)|\nCLÁUSULA|\nAssinatura|$)/i);
    if (mM) {
      const linhas = mM[1].split('\n').map(l=>l.trim()).filter(l=>l && !/^BEBIDA|^ESTIM|^VALOR|^ESTIMATIVA/i.test(l));
