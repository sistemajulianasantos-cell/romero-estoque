...
var pageInfo={
  ...
  regras:['Regras e Cálculos','Fichas de coquetéis e regras de proporção'],
...
var ACESSO = {
  admin:       ['estoque','contratos','regras','producao','separacao','agenda','financeiro','analise'],
...
function extrairDadosContratoRomero(txt) {
  ...
  // ── CARDÁPIO ─────────────────────────────────────────────────────────────
  // Pega tudo da letra e) até a letra f) — cláusula 2.1 do contrato Romero
  // Inclui coquetéis, shots, opcionais (receptivo, champagne, etc.)
  {
    let cardapioRaw = '';
    // Normalizar: substituir quebras de linha por marcador para facilitar regex
    const tNorm = t.replace(/\r/g,'');

    // Tentativa 1: 
e) ... 
f) com quebras de linha
    const m1 = tNorm.match(/
e\)[^
]*fornecimento[^
]*
([\s\S]*?)
f\)\s*Responsabilizar/i);
    if (m1) cardapioRaw = m1[1];

    // Tentativa 2: e) sem quebra até f) (PDF em linha)
    if (!cardapioRaw) {
      const m2 = tNorm.match(/e\)\s*Responsabilizar[^e]*?fornecimento[^e]*?([\s\S]*?)(?=f\)\s*Responsabilizar|f\)\s*Provid)/i);
      if (m2) cardapioRaw = m2[1];
    }

    // Tentativa 3: a partir de Coquetéis até f)
    if (!cardapioRaw) {
      const m3 = tNorm.match(/Coquet[eé]is?[\s\S]*?(?=
f\)\s*Responsabilizar|
f\)\s*Provid|CLÁUSULA\s*[23])/i);
      if (m3) cardapioRaw = m3[0];
    }

    // Tentativa 4: texto em linha sem quebras — separar por letras de alínea
    if (!cardapioRaw) {
      const m4 = tNorm.match(/e\)[\s\S]*?Coquet[eé]is?:?(.*?)(?=f\)\s*Responsabilizar)/i);
      if (m4) cardapioRaw = m4[1];
    }

    if (cardapioRaw) {
      // Se o cardápio veio em linha, tentar separar pelos traços dos coquetéis
      let linhas;
      if (cardapioRaw.includes('\n')) {
        linhas = cardapioRaw.split('\n');
      } else {
        // Linha única — separar por " - " que precede cada coquetel
        linhas = cardapioRaw.split(/(?=\s-\s[A-ZÁÉÍÓÚÂÊÎÔÛÃÕ])/);
      }
      d.cardapio = linhas
        .map(l => l.trim())
        .filter(l => l.length > 1)
        .filter(l => !/^f\)|^g\)|^CLÁUSULA\s*[23]|^Responsabilizar/i.test(l))
        .join('\n')
        .trim();
    }
  }
...
function rSepNova() {
  ...
  // Preferir produções cadastradas; fallback para contratos direto
  const producoes = (D.producoes||[]).sort((a,b)=>(b.data||'').localeCompare(a.data||''));
  const contratos = (D.contratos||[]).sort((a,b)=>(b.dataEvento||'').localeCompare(a.dataEvento||''));

  const optsProducoes = producoes.map(p=>`<option value="prod:${p.id}">📋 ${p.evento||p.cliente||'—'} · ${fd(p.data)||''}</option>`).join('');
  const optsContratos = contratos.map(c=>`<option value="cont:${c.id}">📄 ${c.nomeCliente||c.nome||'—'} · ${c.tipoEvento||''} · ${fd(c.dataEvento)||''}</option>`).join('');

  cont.innerHTML = `
    <div style="padding:16px">
      <label class="lbl" style="display:block;margin-bottom:6px">Selecionar evento *</label>
      <select id="sep-prod-sel" class="inp" onchange="sepCarregarEvento(this.value)" style="width:100%;max-width:560px">
        <option value="">— Selecione o evento —</option>
        ${optsProducoes ? '<optgroup label="── Produções cadastradas ──">' + optsProducoes + '</optgroup>' : ''}
        ${optsContratos ? '<optgroup label="── Contratos (sem produção) ──">' + optsContratos + '</optgroup>' : ''}
      </select>
      <div style="font-size:10px;color:var(--text3);margin-top:4px">Produções cadastradas têm prioridade — dados já revisados</div>
      <div id="sep-form-campos" style="display:none;margin-top:16px"></div>
    </div>`;
}

function sepCarregarEvento(val) {
  if (!val) return;
  if (val.startsWith('prod:')) {
    sepCarregarProducao(val.replace('prod:',''));
  } else if (val.startsWith('cont:')) {
    sepCarregarContrato(val.replace('cont:',''));
  }
}

function sepCarregarContrato(contId) {
  const c = (D.contratos||[]).find(x=>x.id===contId);
  if (!c) return;
  // Montar objeto no formato de produção para reutilizar sepCarregarProducao
  const conv = parseInt(c.convidados)||0;
  const equipe = c.equipe||[];
  const pFake = {
    id: 'cont:'+contId,
    evento: c.nomeCliente||c.nome||'',
    cliente: c.nomeCliente||c.nome||'',
    data: c.dataEvento||'',
    convidados: conv,
    local: c.local||'',
    hrInicio: c.horarioInicio||c.hrInicio||'',
    hrFim: c.horarioFim||c.hrFim||'',
    equipe: equipe,
    equipeTexto: equipe.map(e=>e.qtd+' '+e.cargo).join(' · '),
    cardapio: c.cardapio||'',
    bebidasConsignadas: c.bebidasConsignadas||'',
    bebidasRomero: c.bebidasRomero||'',
    bebidasCliente: c.bebidasCliente||'',
  };
  _sepFakeProducao = pFake;
  sepCarregarProducaoObj(pFake);
}


var _sepFakeProducao = null;

function sepCarregarProducaoObj(p) {
  // Mesma lógica de sepCarregarProducao mas aceita objeto diretamente
  ...
  // Cruzar cardápio com fichas técnicas
  const itensFichas = cruzarCardapioComFichas(p.cardapio||'');

  const cont = document.getElementById('sep-form-campos');
  if (!cont) return;
  cont.style.display = '';

  // Montar lista de itens reconhecidos das fichas
  let fichaSummary = '';
  if (itensFichas && Object.keys(itensFichas).length) {
    const total = Object.values(itensFichas).reduce((s,a)=>s+a.length,0);
    fichaSummary = `<div style="background:var(--green-bg);border:1px solid var(--green-dim);border-radius:var(--radius);padding:8px 12px;margin-bottom:10px;font-size:11px;color:var(--green)">
      ✅ ${total} itens identificados automaticamente a partir das fichas de coquetéis
    </div>`;
  } else if (p.cardapio) {
    fichaSummary = `<div style="background:var(--amber-bg);border:1px solid var(--amber-dim);border-radius:var(--radius);padding:8px 12px;margin-bottom:10px;font-size:11px;color:var(--amber)">
      ⚠️ Cardápio encontrado mas sem fichas cadastradas para os coquetéis. Cadastre fichas em Regras e Cálculos para preenchimento automático.
    </div>`;
  }
  ...
  cont.innerHTML = `
    ...
    ${p.cardapio ? `<div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);padding:10px 14px;margin-bottom:14px;font-size:11px">
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;margin-bottom:6px">Cardápio do contrato</div>
      <div style="white-space:pre-wrap;color:var(--text)">${p.cardapio}</div>
    </div>` : '<div style="color:var(--amber);font-size:11px;padding:8px 0;margin-bottom:10px">⚠️ Cardápio não encontrado no contrato. Preencha manualmente abaixo.</div>'}
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
      ...
      <div><label class="lbl">Gelo Translúcido (UN)</label><input class="inp" type="number" id="sep-gelo-trans" value="0"></div>
      ...
      <div><label class="lbl">Mix Frutas Vermelhas (UN)</label><input class="inp" type="number" id="sep-frutas-verm" value="${c.prodBase.frutasVerm}"></div>
      <div style="grid-column:1/-1"><label class="lbl">Bebidas Alcoólicas</label>
        <textarea class="inp" id="sep-bebidas-alc" rows="3" style="font-size:12px;resize:vertical">${(p.bebidasConsignadas||'')}${p.bebidasRomero?'\n'+p.bebidasRomero:''}${p.bebidasCliente?'\n'+p.bebidasCliente:''}</textarea>
      </div>
      <div style="grid-column:1/-1"><label class="lbl">Coquetéis (editável)</label>
        <textarea class="inp" id="sep-coqueteis" rows="6" style="font-size:12px;resize:vertical">${p.cardapio||''}</textarea>
      </div>
      <div><label class="lbl">Mix Artesanal</label><textarea class="inp" id="sep-mix-artesanal" rows="3" style="font-size:12px;resize:vertical">${itensFichas?.['MIX ARTESANAL']?.join('\n')||''}</textarea></div>
      <div><label class="lbl">Xaropes Especiais</label><textarea class="inp" id="sep-xaropes-esp" rows="3" style="font-size:12px;resize:vertical">${itensFichas?.['XAROPES ESPECIAIS']?.join('\n')||''}</textarea></div>
      <div style="grid-column:1/-1"><label class="lbl">Frutas / Hortifruti</label><textarea class="inp" id="sep-frutas" rows="3" style="font-size:12px;resize:vertical">${itensFichas?.['FRUTAS / HORTIFRUTI']?.join('\n')||''}</textarea></div>
    ...
    <div style="margin-top:14px;background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);padding:12px 14px">
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;margin-bottom:8px">Material automático — ${c.bt} bartender(s) · ${conv} convidados</div>
    ...
    <input type="hidden" id="sep-prod-id" value="${p.id}">
...
function sepCarregarProducao(prodId) {
  ...
  if (!p) return;
  sepCarregarProducaoObj(p);
}

function _sepCarregarProducaoLegado(prodId) {
  if (!prodId) return;
  const p = (D.producoes||[]).find(x=>x.id===prodId);
  if (!p) return;
  const conv = parseInt(p.convidados)||0;
...
// Base de todos os itens possíveis da folha de separação, por categoria
var ITENS_SEPARACAO = {
  "MATERIAL": [
    "TAPETINHO","PRATINHO PRETO","GARRAFA SOUR","GARRAFA SIMPLE","MIXING GLASS",
    "BOWL PRETO","PÁ DE GELO","COQUETELEIRA COMPLETA","FUNIL","TÁBUA BRANCA",
    "FACA","DOSADOR","DESCASCADOR","PENEIRINHA","MINI RALO","BIQUEIRA","BAILARINA",
    "MACERADOR","PINÇA","PEGADOR DE GELO","STREINER","BALDE COPA","ESCORREDOR",
    "DETERGENTE","BUCHA","COOLER REPOSIÇÃO","TÉRMICA PRETA","CAIXA PRETA G",
    "CAIXA PRETA M","SUPORTE PARA TÉRMICA","CAIXA AZUL DE GELO","CAIXA VERDE DE GELO",
    "CUBA PRETA","LIXEIRA","RÁDIO","TESOURA"
  ],
  "ESPECIARIAS": [
    "INFUSÃO","DESIDRATADOS","ANGOSTURA","EMULSIFICANTE","BITTER DE LARANJA",
    "ADOÇANTE","SAL DE PÁPRICAS"
  ],
  "DESCARTÁVEIS": [
    "ÁLCOOL","GUARDANAPO","CANUDO","SACO DE LIXO","COPO DESCARTÁVEL","PALITO DE ACRÍLICO","PANO DE PRATO"
  ],
  "COPOS": [
    ...
    "TUBO DE ENSAIO","TAÇA CALISE AMERICA"
  ],
  "BEBIDAS ALCOÓLICAS": [
    "VODKA ABSOLUT","GIM BEEFEATER","GIM TANQUERAY","TEQUILA","RUM HAVANA",
    "ESPUMANTE LE BLANC","LICOR 43","CAMPARI","JAMESON","VERMUTE CINZANO",
    "APEROL","CACHAÇA SPIRAL","FERNET","BANANINHA","FIREBALL","MANZA"
  ],
  "FRUTAS / HORTIFRUTI": [
    "MORANGO","LIMÃO SICILIANO","GRAPEFRUIT","LIMÃO TAITI","LARANJA BAHIA",
    "UVA VERDE","HORTELÃ","MANJERICÃO","ALECRIM","TOMILHO","PEPINO",
    "FLOR COMESTÍVEL","LIMÃO CAPETA","MARACUJÁ","PERA","MAÇÃ VERDE","PEPINO"
  ],
  "BEBIDAS SEM ÁLCOOL": ["ÁGUA TÔNICA","ÁGUA COM GÁS"],
  "MIX ARTESANAL": [
    "SODA GINGER ALE","SODA GRAPEFRUIT","ESPUMA DE GENGIBRE","ESPUMA DE LIMÃO SICILIANO",
    "ESPUMA DE LIMÃO","FRUTAS VERMELHAS","XAROPE DE AÇÚCAR","SUCO DE LIMÃO"
  ],
  "XAROPES ESPECIAIS": [
    "LICOR DE CAFÉ","PURE MONIN PERA","LICHIA","MORANGO XP","MARACUJÁ XP",
    "TANGERINA","CARAMELO","CUCUMBER","PÊSSEGO","FRAMBOESA","BAUNILHA","AÇÚCAR DE BAUNILHA"
  ],
  "PRODUÇÃO": [
    "SUCO DE LIMÃO (LT)","XAROPE DE AÇÚCAR (LT)","MIX FRUTAS VERMELHAS",
    "CAFÉ","MIX AÇÚCAR DE BAUNILHA E MARACUJÁ"
  ]
};

// Kit base sempre presente (proporcional à equipe, não ao cardápio)
var KIT_BASE_BARTENDER = [
  "TAPETINHO","PRATINHO PRETO","MIXING GLASS","BOWL PRETO","PÁ DE GELO",
  "COQUETELEIRA COMPLETA","FACA","DOSADOR","PENEIRINHA","BIQUEIRA","BAILARINA",
  "PINÇA","PEGADOR DE GELO","STREINER","GARRAFA SOUR","GARRAFA SIMPLE"
];
...
function setRegrasView(v) {
  ['fichas','regras-prop','nova-ficha'].forEach(x => {
    const el = document.getElementById('regras-view-' + x);
    if (el) el.style.display = x === v ? '' : 'none';
    const btn = document.getElementById('regras-tab-' + x);
    if (btn) btn.classList.toggle('active', x === v);
  });
  if (v === 'fichas') rFichas();
  if (v === 'regras-prop') rRegrasProp();
  if (v === 'nova-ficha') rFormFicha();
}

// ── Fichas de Coquetéis ───────────────────────────────
function rFichas() {
  const cont = document.getElementById('fichas-body');
  if (!cont) return;
  const fichas = D.fichas || [];
  if (!fichas.length) {
    cont.innerHTML = '<div style="text-align:center;color:var(--text3);padding:32px;font-size:13px">Nenhuma ficha cadastrada. Clique em "+ Nova Ficha" para começar.<br><br>Dica: cadastre seus coquetéis mais usados e o sistema preencherá a separação automaticamente.</div>';
    return;
  }
  cont.innerHTML = fichas.map(f => {
    const itens = f.itens || [];
    const porCat = {};
    itens.forEach(i => { if(!porCat[i.cat]) porCat[i.cat]=[]; porCat[i.cat].push(i.nome); });
    return `
      <div class="sec" style="margin-bottom:10px">
        <div class="sec-head" style="display:flex;align-items:center;gap:10px">
          <span class="sec-title">🍹 ${f.nome}</span>
          ${f.variantes?`<span style="font-size:11px;color:var(--text3)">${f.variantes}</span>`:''}
          <div style="margin-left:auto;display:flex;gap:6px">
        ...
        <div style="padding:8px 16px;display:flex;flex-wrap:wrap;gap:12px;font-size:11px">
          ${Object.entries(porCat).map(([cat, nomes]) =>
            `<div><span style="color:var(--text3);font-weight:600">${cat}:</span> ${nomes.join(', ')}</div>`
          ).join('')}
        </div>
      </div>`;
  }).join('');
...
function rFormFicha(fichaExistente) {
  const cont = document.getElementById('regras-view-nova-ficha');
  if (!cont) return;
  const f = fichaExistente || { nome:'', variantes:'', itens:[] };
  const itensIds = new Set(f.itens.map(i => i.cat+'|'+i.nome));
      ...
      <div style="padding:14px 16px">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
          <div><label class="lbl">Nome do Coquetel *</label>
            <input class="inp" id="fc-nome" type="text" placeholder="Ex: FITZGERALD" value="${f.nome}" style="text-transform:uppercase">
          </div>
          <div><label class="lbl">Variantes / Aliases</label>
            <input class="inp" id="fc-variantes" type="text" placeholder="Ex: FITZGERAL, FITZ" value="${f.variantes||''}">
            <div style="font-size:10px;color:var(--text3);margin-top:2px">Nomes alternativos separados por vírgula</div>
          </div>
        </div>
        <div style="font-size:11px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px">
          Marque os itens que este coquetel precisa
        </div>
        <div id="fc-itens-container">
          ${Object.entries(ITENS_SEPARACAO).map(([cat, itens]) => `
            <div style="margin-bottom:14px">
              ...
              <div style="display:flex;flex-wrap:wrap;gap:8px">
                ${itens.map(item => {
                  const key = cat+'|'+item;
                  const checked = itensIds.has(key) ? 'checked' : '';
                  return `<label style="display:flex;align-items:center;gap:5px;font-size:12px;cursor:pointer;background:var(--bg3);padding:4px 10px;border-radius:var(--radius);border:1px solid var(--border)">
                    <input type="checkbox" data-cat="${cat}" data-nome="${item}" ${checked} style="cursor:pointer">
                    ${item}
                  </label>`;
                }).join('')}
              </div>
        ...
        <div style="margin-top:8px">
          <label class="lbl" style="display:block;margin-bottom:4px">Item personalizado (não listado acima)</label>
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px">
            <select id="fc-custom-cat" class="inp" style="width:180px">
              ${Object.keys(ITENS_SEPARACAO).map(cat=>`<option value="${cat}">${cat}</option>`).join('')}
            </select>
            <input class="inp" id="fc-custom-nome" type="text" placeholder="Nome do item" style="flex:1;min-width:180px">
        ...
        <div style="display:flex;gap:8px;margin-top:16px">
          <button class="btn" onclick="salvarFicha('${fichaExistente?.id||''}')" style="background:var(--green)">💾 Salvar Ficha</button>
          <button class="btn" onclick="setRegrasView('fichas')" style="background:var(--bg3);color:var(--text)">Cancelar</button>
        </div>
      </div>
    </div>`;

  // Restaurar itens customizados
  if (fichaExistente) {
    const todosItens = new Set(Object.entries(ITENS_SEPARACAO).flatMap(([cat,itens])=>itens.map(i=>cat+'|'+i)));
    fichaExistente.itens.filter(i=>!todosItens.has(i.cat+'|'+i.nome)).forEach(i=>{
...
function adicionarItemCustom() {
  const cat = document.getElementById('fc-custom-cat')?.value;
  const nome = document.getElementById('fc-custom-nome')?.value?.trim().toUpperCase();
  if (!nome) return;
...
function adicionarTagCustom(cat, nome) {
  ...
  _customItens.push({id, cat, nome});
  const cont = document.getElementById('fc-custom-lista');
  if (!cont) return;
...
function removerCustom(id, el) {
  _customItens = _customItens.filter(x=>x.id!==id);
  el?.remove();
}

function salvarFicha(idExistente) {
  const nome = document.getElementById('fc-nome')?.value?.trim().toUpperCase();
  if (!nome) { alert('Preencha o nome do coquetel.'); return; }
  const variantes = document.getElementById('fc-variantes')?.value?.trim() || '';

  // Coletar itens marcados
  const itens = [];
  document.querySelectorAll('#fc-itens-container input[type="checkbox"]:checked').forEach(cb => {
    itens.push({ cat: cb.dataset.cat, nome: cb.dataset.nome });
  });
  // Adicionar customizados
  _customItens.forEach(i => itens.push({ cat: i.cat, nome: i.nome }));

  if (!D.fichas) D.fichas = [];
  if (idExistente) {
    const idx = D.fichas.findIndex(f=>f.id===idExistente);
    if (idx>=0) D.fichas[idx] = { ...D.fichas[idx], nome, variantes, itens };
  } else {
    D.fichas.push({ id:'FIC'+Date.now(), nome, variantes, itens, criadoEm: new Date().toISOString() });
  }
  _customItens = [];
  sv('fichas');
  alert('Ficha salva!');
...
function editarFicha(id) {
  const f = (D.fichas||[]).find(x=>x.id===id);
  if (!f) return;
  _customItens = [];
  setRegrasView('nova-ficha');
  setTimeout(() => rFormFicha(f), 50);
}

function excluirFicha(id) {
  if (!confirm('Excluir esta ficha?')) return;
  D.fichas = (D.fichas||[]).filter(f=>f.id!==id);
  sv('fichas'); rFichas();
}

// ── Regras de Proporção ───────────────────────────────
var REGRAS_PADRAO = [
  { id:'r001', nome:'Bartenders por convidados', valor:30, unidade:'convidados/bartender', descricao:'1 bartender a cada X convidados' },
  { id:'r002', nome:'Gelo cubo base', valor:5, unidade:'convidados/pacote', descricao:'1 pacote de gelo para cada X convidados (ajustar por horário)' },
  { id:'r003', nome:'Água tônica base', valor:5, unidade:'convidados/unidade', descricao:'1 unidade para cada X convidados (ajustar por cardápio)' },
  { id:'r004', nome:'Água com gás base', valor:5, unidade:'convidados/unidade', descricao:'1 unidade para cada X convidados (ajustar por cardápio)' },
  { id:'r005', nome:'Suco de limão base', valor:20, unidade:'convidados/litro', descricao:'1 litro para cada X convidados (ajustar por cardápio)' },
  { id:'r006', nome:'Xarope de açúcar base', valor:20, unidade:'convidados/litro', descricao:'1 litro para cada X convidados (ajustar por cardápio)' },
];

function rRegrasProp() {
  const cont = document.getElementById('regras-prop-body');
  if (!cont) return;
  const regras = D.regrasProp && D.regrasProp.length ? D.regrasProp : REGRAS_PADRAO;
  cont.innerHTML = `
    <div style="font-size:12px;color:var(--text3);margin-bottom:14px">
      Estas regras são usadas para calcular quantidades base na folha de separação. Ajuste os valores conforme a experiência da Romero.
    </div>
    <div style="display:grid;gap:10px">
      ${regras.map(r => `
        <div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);padding:12px 16px;display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center">
          <div>
            <div style="font-size:13px;font-weight:600;color:var(--text)">${r.nome}</div>
            <div style="font-size:11px;color:var(--text3);margin-top:2px">${r.descricao}</div>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="font-size:11px;color:var(--text3)">1 ${r.unidade.split('/')[1]} para</span>
            <input class="inp" type="number" value="${r.valor}" min="1" style="width:70px;text-align:center;font-weight:700;font-family:var(--mono)"
              onchange="salvarRegra('${r.id}',this.value)">
            <span style="font-size:11px;color:var(--text3)">${r.unidade.split('/')[0]}</span>
          </div>
        </div>`).join('')}
    </div>`;
}

function salvarRegra(id, valor) {
  if (!D.regrasProp) D.regrasProp = JSON.parse(JSON.stringify(REGRAS_PADRAO));
  const r = D.regrasProp.find(x=>x.id===id);
  if (r) { r.valor = parseInt(valor)||r.valor; sv('regrasProp'); }
}

// ── Função auxiliar: obter regra ──────────────────────
function getRegra(id) {
  const regras = D.regrasProp && D.regrasProp.length ? D.regrasProp : REGRAS_PADRAO;
  return regras.find(r=>r.id===id)?.valor;
}

// ── Cruzar cardápio com fichas ────────────────────────
function cruzarCardapioComFichas(cardapioTexto) {
  if (!cardapioTexto || !(D.fichas||[]).length) return null;
  const fichas = D.fichas || [];
  const linhas = cardapioTexto.split('\n').filter(Boolean);
  const itensNecessarios = {}; // cat -> Set de nomes

  linhas.forEach(linha => {
    const linhaNorm = linha.toUpperCase().replace(/[-•*]/g,'').trim();
    fichas.forEach(ficha => {
      // Verificar se o nome ou algum alias aparece na linha
      const nomes = [ficha.nome, ...(ficha.variantes||'').split(',').map(v=>v.trim())].filter(Boolean);
      const encontrou = nomes.some(n => linhaNorm.includes(n.toUpperCase()));
      if (encontrou && ficha.itens) {
        ficha.itens.forEach(item => {
          if (!itensNecessarios[item.cat]) itensNecessarios[item.cat] = new Set();
          itensNecessarios[item.cat].add(item.nome);
        });
      }
    });
  });
  ...
  const resultado = {};
  Object.entries(itensNecessarios).forEach(([cat, set]) => {
    resultado[cat] = [...set];
  });
