...
function extrairDadosContratoRomero(txt) {
  ...
  // ── BEBIDAS ROMERO ou CONSIGNADAS — cláusula 2, letra m) ────────────────
  // Usa txt bruto para evitar problemas da normalização de letras
  {
    const txtBruto = txt || t;
    const mMhdr = txtBruto.match(/m\s*\)[^\n]*Bebidas fornecidas pela ROMERO([^\n]*)/i);
    const isConsignada2 = mMhdr ? /CONSIGNADA/i.test(mMhdr[1]) : false;
    // Capturar todas as linhas após o cabeçalho da letra m)
    const mM = txtBruto.match(/m\s*\)[^\n]*Bebidas fornecidas pela ROMERO[^\n]*\n([\s\S]*?)(?=\n\s*[a-z]\s*\)|CLÁUSULA\s*3|$)/i);
    if (mM) {
      const linhasBeb = mM[1].split('\n')
        .map(function(l){return l.trim();})
        .filter(function(l){
          if (!l) return false;
          if (/^(BEBIDA|ESTIMATIVA|VALOR|ESTIMATIVA DE BEBIDAS)/i.test(l)) return false;
          return true;
        })
        .map(function(l){
          // Remover estimativas: "03 A 04 GARRAFAS R$89,90"
          return l.replace(/\s+\d+\s+A\s+\d+\s+GARRAFAS?[^\n]*/i,'')
            .replace(/\s+\d+\s+GARRAFAS?[^\n]*/i,'')
            .replace(/\s+R\$[\d.,]+\s*$/,'')
            .trim();
        })
      ...
      if (linhasBeb.length) {
        if (isConsignada2) {
          d.bebidasConsignadas = linhasBeb.join('\n').trim();
        } else {
          d.bebidasRomero = linhasBeb.join('\n').trim();
        }
      }
    }
  }
