...
window.QUEBRAS_PRELOAD = QUEBRAS_PRELOAD;
...
window.FESTAS_PRELOAD = FESTAS_PRELOAD;
...
function excluirContrato(id) {
  ...
  D.agenda     = (D.agenda    ||[]).filter(a => (a.contratoId||'') !== id);
  D.financeiro = (D.financeiro||[]).filter(f => (f.contratoId||'') !== id);
  D.producoes  = (D.producoes ||[]).filter(p => (p.contratoId||'') !== id);
