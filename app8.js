...
function setRegrasView(v) {
  ['fichas','proporcoes','nova-ficha'].forEach(function(x) {
...
function rFormFicha(fichaExistente) {
  ...
  Object.entries(ITENS_FOLHA).forEach(function(entry) {
    ...
    '<div style="margin-top:8px">' +
      ...
      '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px">' +
        '<select id="fc-custom-cat" class="inp" style="width:180px">' +
          Object.keys(ITENS_FOLHA).map(function(cat){return '<option value="'+cat+'">'+cat+'</option>';}).join('') +
        '</select>' +
    ...
    (D.fichas||[]).forEach(function(ficha) {
      ...
      if (encontrou && ficha.itens) {
        ficha.itens.forEach(function(item) {
          ...
          resultado[item.cat][item.nome] = (resultado[item.cat][item.nome]||0) + 1;
        });
      }
    });
  });
...
function sepCarregarProducao(prodId) {
  ...
  var fichasNaoEncontradas = [];

  // Identificar coquetéis sem ficha
  if (p.cardapio) {
    var linhas = p.cardapio.split('\n').filter(Boolean);
    linhas.forEach(function(linha) {
      var linhaNorm = linha.toUpperCase().replace(/[-•*:]/g,'').trim();
      if (!linhaNorm || linhaNorm.length < 3) return;
      var temFicha = (D.fichas||[]).some(function(ficha){
        var nomes = [ficha.nome].concat((ficha.variantes||'').split(',').map(function(v){return v.trim();})).filter(Boolean);
        return nomes.some(function(n){return linhaNorm.includes(n.toUpperCase());});
      });
      if (!temFicha && linhaNorm.startsWith('-')) {
        fichasNaoEncontradas.push(linha.trim());
      }
    });
  }
  ...
  var equipeHtml = equipe.length ? equipe.map(function(e){return '<span style="margin-right:10px">'+e.qtd+' '+e.cargo+'</span>';}).join('') : (p.equipeTexto||'—');
  ...
  // Aviso de fichas não encontradas
  if (fichasNaoEncontradas.length) {
    html += '<div style="background:var(--amber-bg);border:1px solid var(--amber-dim);border-radius:var(--radius);padding:8px 12px;margin-bottom:12px;font-size:11px;color:var(--amber)">' +
      '⚠️ Coquetéis sem ficha cadastrada (quantidades não calculadas): ' +
      fichasNaoEncontradas.join(', ') +
      ' <button class="btn-sm" onclick="setRegrasView(\'nova-ficha\');setSepView(\'lista\')" style="margin-left:8px">Cadastrar fichas</button>' +
    '</div>';
  }

  if (temFichas) {
    html += '<div style="background:var(--green-bg);border:1px solid var(--green-dim);border-radius:var(--radius);padding:6px 12px;margin-bottom:12px;font-size:11px;color:var(--green)">' +
      '✅ ' + Object.values(itensCardapio).reduce(function(s,c){return s+Object.keys(c).length;},0) + ' itens identificados automaticamente a partir das fichas de coquetéis' +
    '</div>';
  }

  // Mostrar cardápio
  if (p.cardapio) {
    html += '<div style="background:var(--bg3);border-radius:var(--radius);padding:8px 12px;margin-bottom:14px;border:1px solid var(--border)">' +
      '<div style="font-size:10px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">CARDÁPIO DO CONTRATO</div>' +
      '<div style="font-size:11px;white-space:pre-wrap;color:var(--text2)">' + p.cardapio + '</div>' +
    '</div>';
  }

  // Gerar tabela de itens por categoria
  var porCat = {};
  regras.forEach(function(r) {
    // Verificar se deve incluir baseado em soSeCardapio
    ...
    var doCardapio = itensDoCardapio ? true : false;

    porCat[r.cat].push({
      item: r.item, qtd: qtd, doCardapio: doCardapio,
      soSeCardapio: r.soSeCardapio, obs: r.obs||'',
      semFicha: r.soSeCardapio && !doCardapio && r.min > 0
    });
  });

  // Campos de bebidas (texto livre)
  ...
  Object.entries(porCat).forEach(function(entry) {
    ...
    html += '<div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden">' +
      '<div style="padding:8px 12px;border-bottom:1px solid var(--border);font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.8px">' + cat + '</div>' +
      '<div style="padding:8px 0">';

    itens.forEach(function(it) {
      var bg = it.doCardapio ? 'background:rgba(61,220,132,.08);' : it.semFicha ? 'background:rgba(247,195,90,.06);' : '';
      var badge = it.doCardapio ? '<span style="font-size:9px;background:var(--green-bg);color:var(--green);border:1px solid var(--green-dim);padding:1px 6px;border-radius:10px;margin-left:6px">✅ cardápio</span>' :
                  it.semFicha   ? '<span style="font-size:9px;background:var(--amber-bg);color:var(--amber);border:1px solid var(--amber-dim);padding:1px 6px;border-radius:10px;margin-left:6px">⚠️ verificar</span>' : '';
      html += '<div style="display:grid;grid-template-columns:1fr 100px;gap:8px;align-items:center;padding:5px 12px;border-bottom:1px solid var(--border);' + bg + '">' +
        ...
        '<input type="number" value="' + it.qtd + '" min="0" ' +
          ...
          'style="font-size:12px;font-weight:600;padding:4px 8px;border-radius:4px;border:1px solid var(--border2);background:var(--bg);color:' + (it.doCardapio?'var(--green)':it.semFicha?'var(--amber)':'var(--text)') + ';text-align:center;font-family:var(--mono)">' +
      '</div>';
    });
  ...
  // Bebidas alcoólicas (texto livre)
  var bebidasAlc = [p.bebidasConsignadas, p.bebidasRomero, p.bebidasCliente].filter(Boolean).join('\n');
  html += '<div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden">' +
    '<div style="padding:8px 12px;border-bottom:1px solid var(--border);font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.8px">BEBIDAS ALCOÓLICAS</div>' +
    '<div style="padding:8px 12px"><textarea class="inp" id="sep-bebidas-alc" rows="4" style="font-size:12px;resize:vertical;width:100%">' + bebidasAlc + '</textarea></div>' +
  '</div>';

  // Coquetéis
  html += '<div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden">' +
    '<div style="padding:8px 12px;border-bottom:1px solid var(--border);font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.8px">COQUETÉIS</div>' +
    '<div style="padding:8px 12px"><textarea class="inp" id="sep-coqueteis" rows="6" style="font-size:12px;resize:vertical;width:100%">' + (p.cardapio||'') + '</textarea></div>' +
  '</div>';
